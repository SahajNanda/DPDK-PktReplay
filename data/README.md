...
- `mergecap` used to combine all pcap files into one large (52GB) pcap file `combined.pcapng`
- `editcap -F pcap -s 1514 -c [n] combined.pcapng chunk.pcapng` used to get pcapng chunk
...
- For pcap chunk with packets greater than 1514 bytes removed rather than truncated, use `tshark -r combined.pcapng -Y "frame.len <= 1514" -c 1000000 -w filter.pcapng`
    - `-r combined.pcapng` select the pcapng file to take a chunk of
    - `-Y "frame.len <= 1514"` limits packet length to 1514 bytes
    - `-c 1000000` sets number of packets read from input pcapng file
    - `-w filter.pcapng` sets the output pcapng file name
